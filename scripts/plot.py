'''
@file plot.py
@brief Utility designed to visualize performance
    ...
    try:
        with open(file_path, 'r', newline='', encoding='utf-8') as f:
            ...
            if not reader.fieldnames:
                print(
                    f"❌ Error: CSV file '{file_path}' has no headers or is empty")
                return sizes, times, unit

            required_fields = ['sample_size', 'time_value']
            if not all(field in reader.fieldnames for field in required_fields):
                print(
                    f"❌ Error: CSV file '{file_path}' missing required columns. Found: {reader.fieldnames}")
                return sizes, times, unit
            ...
            for row in reader:
                ...
                try:
                    ...
                    if size <= 0 or time < 0:
                        print(
                            f"⚠️ Warning: Invalid data in row {row_count} of {file_path}: size={size}, time={time}")
                        continue
                    ...
                    print(
                        f"⚠️ Skipping row {row_count} in {file_path} due to error: {e}")
                    continue

            if row_count == 0:
                print(
                    f"⚠️ Warning: CSV file '{file_path}' contains no data rows")
...
def read_json(file_path: Path) -> Tuple[List[int], List[float], str]:
    ...
    if not isinstance(data, list):
        print(
            f"❌ Error: JSON file '{file_path}' should contain a list of objects")
        return sizes, times, unit
    ...
    for entry in data:
        ...
        try:
            if not isinstance(entry, dict):
                print(
                    f"⚠️ Skipping non-dictionary entry {entry_count} in {file_path}")
                continue
            ...
            if size <= 0 or time < 0:
                print(
                    f"⚠️ Warning: Invalid data in entry {entry_count} of {file_path}: size={size}, time={time}")
                continue
            ...
            print(
                f"⚠️ Skipping entry {entry_count} in {file_path} due to error: {e}")
            continue

    if entry_count == 0:
        print(f"⚠️ Warning: JSON file '{file_path}' contains no valid entries")
...
def validate_data(sizes: List[int], times: List[float], filename: str) -> bool:
    ...
    if len(sizes) != len(times):
        print(
            f"❌ Error: Mismatched data length in {filename} (sizes: {len(sizes)}, times: {len(times)})")
        return False
    ...
    if sorted(sizes) != sizes:
        print(
            f"⚠️ Warning: Sample sizes in {filename} are not sorted. Sorting will be applied.")
...
def plot_metrics(
    ...
    if len(units) > 1:
        print(
            f"⚠️ Warning: Multiple time units detected: {units}. Using first unit.")
    common_unit = datasets[0]['unit'] if datasets else "units"

    # Plot each dataset
    for i, dataset in enumerate(datasets):
        if not dataset['sizes'] or not dataset['times']:
            print(f"⚠️ Skipping empty dataset: {dataset['label']}")
            continue
...
def main() -> None:
    ...
    for file_path in file_paths:
        ...
        if file_path.suffix.lower() not in {'.csv', '.json'}:
            print(
                f"❌ Error: Unsupported file format for '{file_path}'. Use .csv or .json")
            continue
        ...
        try:
            ...
            if validate_data(sizes, times, file_path.name):
                ...
                print(
                    f"✅ Successfully loaded {len(sizes)} data points from {file_path.name}")
    ...
    print(f"📊 Total datasets to plot: {len(valid_datasets)}")
